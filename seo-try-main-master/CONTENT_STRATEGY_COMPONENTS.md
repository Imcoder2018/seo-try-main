...
├── ContentStrategyDashboard (Main UI)
│   ├── Overview Tab
│   ├── Keywords Tab
│   ├── Content Gaps Tab
│   ├── Pages Tab
│   ├── Page Details Tab
│   ├── Suggestions Tab
│   └── Planner Tab → PlannerView Component
│
├── AutoContentEngine (Auto Content Tab)
│
└── HistoryPanel (Slide-out panel)

API Layer:
├── /api/content/analyze (Content analysis)
├── /api/content/history (Analysis history)
├── /api/content/auto-plan (Calendar planning)
├── /api/content/generate (Content creation)
└── /api/content/schedule (Publishing)

Trigger.dev Tasks:
├── content-analyzer (Analyzes website content)
├── content-extractor (Extracts content from URLs)
├── content-generator (Generates AI content)
└── auto-discovery (Finds content opportunities)
```
...
1. **Page Load** → `content-strategy/page.tsx` initializes
2. **Website Analysis** → Triggers `content-analyzer` task → Updates dashboard
3. **Content Planning** → Drag items in `PlannerView` → Creates calendar events
4. **Content Generation** → `AutoContentEngine` → Uses generation APIs
5. **History Access** → `HistoryPanel` → Fetches from history API
